...
#include <x265.h>
#include <float.h>
#include <stdio.h> // included to patch ffmpeg for semantic compression
...
// this function is generated to utilize in semantic compression project
static int inject_rois_from_file(AVCodecContext *avctx, AVFrame *frame, const char *filename) {
    static FILE *fp = NULL;
    ...
    if (!fp) {
        fp = fopen(filename, "r");
        if (!fp) {
            av_log(avctx, AV_LOG_ERROR, "Failed to open ROI file: %s\n", filename);
            return 0;
        }
    }
    ...
    if (!fgets(line, sizeof(line), fp)) {
        av_log(avctx, AV_LOG_INFO, "End of ROI file reached or error.\n");
        return 0; // No ROI for this frame
    }

    int max_rois = 2; // TODO adjust as needed, it can be also given as in the CLI for an automatic adjustment
    AVRegionOfInterest *roi;
    AVFrameSideData *sd = av_frame_new_side_data(frame, AV_FRAME_DATA_REGIONS_OF_INTEREST, sizeof(AVRegionOfInterest) * max_rois);
    if (!sd) return AVERROR(ENOMEM);
    roi = (AVRegionOfInterest *)sd->data;
    memset(roi, 0, sizeof(AVRegionOfInterest) * max_rois); //Trial code to fix the error obtained.
    int i = 0;
    char *token = strtok(line, ";");
    while (token && i < max_rois) {
        int left, top, right, bottom, num, den;
        if (sscanf(token, "%d,%d,%d,%d,%d,%d", &left, &top, &right, &bottom, &num, &den) == 6) {
            printf("%d,%d,%d,%d,%d,%d\n", left, top, right, bottom, num, den);
            roi[i].self_size = sizeof(AVRegionOfInterest);
            roi[i].left = left;
            roi[i].top = top;
            roi[i].right = right;
            roi[i].bottom = bottom;
            roi[i].qoffset.num = num;
            roi[i].qoffset.den = den;
            i++;
        }
        token = strtok(NULL, ";");
    }
    if (i<max_rois){
        printf("ERROR: ZERO INITIALIZED AND NOT UTILIZED ROIS DEFINED.\nIf max rois are higher than the rois utilized, it will give an error for zero denum as the denum initialized 0 for the non-assigned rois. It starts from the final defined RoI and prioritizes the initial ones.\n");
    }
    av_log(avctx, AV_LOG_INFO, "----- Final ROI List for Frame %d -----\n", frame_index);
    for (int j = 0; j < i; j++) {
        av_log(avctx, AV_LOG_INFO,
            "ROI[%d]: left=%d, top=%d, right=%d, bottom=%d, qoffset=%d/%d\n",
            j,
            roi[j].left,
            roi[j].top,
            roi[j].right,
    ...
    printf("Line 658: success.\n");
    av_log(avctx, AV_LOG_INFO, "--------------------------------------\n");
    av_log(avctx, AV_LOG_DEBUG, "Injected %d ROIs into frame %d\n", i, frame_index++);
    printf("The ROI den before the return is: \n");
    printf("%d\n", roi[0].qoffset.den);
    // if (sd){
    //     printf("sd is True in the first function\n");
    // }
    return 0;
}
    ...
    if (sd) {
        if (ctx->params->rc.aqMode == X265_AQ_NONE) {
            ...
            for (int i = nb_rois - 1; i >= 0; i--) {
                ...
                printf("Here is the denumerator revealed:\n");
                printf("%d\n",roi->qoffset.den);
                ...
                printf("Line 698 passed/n");
                qoffset = roi->qoffset.num * 1.0f / roi->qoffset.den;
                printf("Line 700 passed/n");
                qoffset = av_clipf(qoffset * qp_range, -qp_range, +qp_range);
                printf("Line 702passed/n");
                for (int y = starty; y < endy; y++)
                    for (int x = startx; x < endx; x++)
                        qoffsets[x + y*mbx] = qoffset;
                // The below code is the modified version to check the adaptive quantization of x265
                // printf("Applying the adaptive quantization for h265...\n");
                // for (int y = starty; y < endy; y++) {
                //     for (int x = startx; x < endx; x++) {
                //         if (x < mbx / 2)
                //             qoffsets[x + y * mbx] = +1.0f * qp_range; // Force low quality left
                //         else
                //             qoffsets[x + y * mbx] = -1.0f * qp_range; // Force high quality right
                //     }
                // }
                                ...
                                const AVFrame *pic, int *got_packet)
{
    ...
    if (pic) {
        // Modifying here to test whether ROI would work.
        // AVRegionOfInterest *roi;
        // AVFrameSideData *sd = av_frame_new_side_data(pic, AV_FRAME_DATA_REGIONS_OF_INTEREST, sizeof(AVRegionOfInterest));
        // roi = (AVRegionOfInterest *)sd->data;
        // roi->self_size = sizeof(AVRegionOfInterest);
        // roi->top = 0; roi->bottom = pic->height/2;
        // roi->left = 0; roi->right = pic->width / 2;
        // roi->qoffset.num = 10; roi->qoffset.den = 1;
        // modification would end here
        AVFrameSideData *sd;
        ReorderedData *rd;
        int rd_idx;
        ...
        x265pic.sliceType = pic->pict_type == AV_PICTURE_TYPE_I ?
                            ...
                            X265_TYPE_AUTO;
        // printf("Line 812: success\n");
        inject_rois_from_file(avctx, (AVFrame *)pic, "/home/ob3942/repos/qcffmpeg/roi.txt");
        //  TODO set this location from the cli of the ffmpeg
        // AVFrame *local_frame = av_frame_clone(pic);
        // if (!sd){
        // printf("sd is not set\n");
        // }

        //inject_rois_from_file(avctx, local_frame, "/home/ob3942/repos/qcffmpeg/roi.txt");

        // if (!sd){
        // printf("sd is not set\n");
        // }
        // printf("Line 813: success\n");
        ret = libx265_encode_set_roi(ctx, pic, &x265pic);

        // if (!sd){
        // printf("sd is not set\n");
        // }
        // printf("Line 815: success\n");
        if (ret < 0)
            return ret;
